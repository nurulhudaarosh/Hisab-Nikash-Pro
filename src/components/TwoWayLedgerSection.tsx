        ...
        {filteredLedgers.length === 0 ? (
          <div className="text-center py-10 px-4 text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-950/40 rounded-2xl border border-slate-200 dark:border-slate-800/60 flex flex-col items-center justify-center space-y-2">
            ...
            <button
              ...
              className="mt-1 px-3.5 py-1.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs transition-all hover:scale-105"
            >
            ...
            {filteredLedgers.map((ledger) => {
              ...
              return (
                <div
                  ...
                  className={`bg-white dark:bg-slate-950/70 border rounded-2xl p-4 transition-all duration-200 hover:shadow-md flex flex-col justify-between group ${
                    isSettled
                ...
                >
                  <div>
                    {/* Top Row: Name, Contact Badge, Type Badge */}
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <h4 className="text-sm font-extrabold text-slate-900 dark:text-white leading-tight group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                              {ledger.name}
                            </h4>
                            {isSettled ? (
                              <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-emerald-100 dark:bg-emerald-500/15 border border-emerald-300 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400 font-mono">
                                Settled
                              </span>
                            ) : (
                              <span
                              ...
                              >
                                {isTheyOwe ? 'Paona (পাওনা)' : 'Dena (দেনা)'}
                              </span>
                            )}
                          </div>
                          <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium capitalize block mt-0.5">
                            {isShop ? '🏪 Grocery / Store' : '👤 Personal Friend'} •{' '}
                            {ledger.history?.length || 0} entries
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <div className="text-[10px] text-slate-500 dark:text-slate-400 font-mono font-medium">
                          Total Dues: {formatCurrency(ledger.originalAmount, settings.currency, isPrivacyMasked)}
                        </div>
                      </div>
                    </div>

                    {/* Progress Bar */}
                    <div className="w-full bg-slate-100 dark:bg-slate-900 rounded-full h-1.5 overflow-hidden my-2.5 border border-slate-200/50 dark:border-slate-800">
                      <div
                  ...
                  <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 mt-1 flex items-center justify-between gap-1.5 flex-wrap">
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className="px-2.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-bold transition-all hover:scale-105 active:scale-95 cursor-pointer flex items-center gap-1"
                        title="Add more due or record payment"
                      >
                        <Receipt className="w-3.5 h-3.5 text-indigo-500" />
                        <span>Manage / Transact</span>
                      </button>

                      {/* WhatsApp Share Button */}
                      <button
                        onClick={() => openShareModal(ledger)}
                        className="p-1.5 rounded-xl bg-emerald-50 dark:bg-slate-900 hover:bg-emerald-100 dark:hover:bg-slate-800 border border-emerald-200 dark:border-slate-800 text-emerald-700 dark:text-emerald-400 text-xs transition-all flex items-center gap-1 cursor-pointer"
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {ledger.history && ledger.history.length > 0 && (
                        <button
                          ...
                          className="px-2 py-1 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 flex items-center gap-0.5 cursor-pointer font-semibold rounded-lg bg-slate-50 dark:bg-slate-900"
                        >
                  ...
                  {isExpanded && ledger.history && ledger.history.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800/80 space-y-2">
                      <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1">
                        <Clock className="w-3 h-3 text-slate-400" /> Transaction Timeline (লেনদেন হিস্ট্রি):
                      </span>
                      <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                        {ledger.history.map((h, idx) => {
                          ...
                          return (
                            <div
                              key={h.id || idx}
                              className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-900 px-3 py-2 rounded-xl text-slate-700 dark:text-slate-300 border border-slate-200/60 dark:border-slate-800"
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[10px] px-1.5 py-0.2 rounded font-bold bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
